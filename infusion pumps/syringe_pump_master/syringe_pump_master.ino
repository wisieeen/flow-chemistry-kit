...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (ttopic=="pump_master/speed"){ //actually time distance between steps
    speed_1 = pp.toInt();
    Serial.print("pump/speed activated: ");
    Serial.println(speed_1);
  }

  else if (ttopic=="pump_master/steps"){
  ...
  else if (ttopic=="pump_master/end_position"){
    requested_pos_1 = pp.toInt()*2;
  ...
  else if (ttopic=="pump_master/run"){
    next_step_1 = micros();
    Serial.print("pump will run in next_step_1: ");
    Serial.println(next_step_1);
  }

  else if (ttopic=="pump_master/run_program"){
    program_0 = program_1;
  ...
  else if (ttopic=="pump_master/program"){
    deserializeJson(program_1, pp); //Parse message
    //Serial.print("program received: ");
    //Serial.println(int(program_1["0"][0]));
  }

  else if (ttopic=="pump_master/homing"){
    homing();
  }

  else if (ttopic=="pump_master/program_length"){
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("esp_pump_3")) {
      Serial.println("connected");
      // Once connected, publish an announcement...
      client.publish("common/status","syringe pump_master connected");
      char* a = (char*)absolute_pos_1;
      client.publish("pump_master/position_now",a);
      // ... and resubscribe
      client.subscribe("pump_master/speed");
      client.subscribe("pump_master/steps");
      client.subscribe("pump_master/end_position");
      client.subscribe("pump_master/program");
      client.subscribe("pump_master/run_program");
      client.subscribe("pump_master/run");
      client.subscribe("pump_master/homing");
      client.subscribe("pump_master/program_length");
  ...
  if(run_program_switch == 1){ //checks if it is program run
    ...
    if(tn <= block_stop){ //checks if block isn't done
      ...
      if (block_num >= program_length){//checks if it was last block
        ...
        client.publish("pump_master/position_now",cstr);
      }
      else{
        block_start = tn;
...
void homing(){
  ...
  absolute_pos_1 = 0;
  client.publish("pump_master/position_now","0");
}
