...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  else if (ttopic=="pump_3/end_position"){
    requested_pos_1 = pp.toInt()*2;
  ...
  else if (ttopic=="pump_3/run"){
    next_step_1 = micros();
    Serial.print("pump will run in next_step_1: ");
    Serial.println(next_step_1);
  }

  else if (ttopic=="pump_3/homing"){
    homing();
  }
  //else if (ttopic=="pump_master/series_length"){
  //  series_length = pp.toInt();
  //}
}
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect("esp_pump_3")) {
      ...
      client.publish("common/status","syringe pump_3 connected");
      char* a = (char*)absolute_pos_1;
      client.publish("pump_3/position_now",a);
      // ... and resubscribe
      client.subscribe("pump_3/speed");
      client.subscribe("pump_3/steps");
      client.subscribe("pump_3/end_position");
      client.subscribe("pump_3/run");
      client.subscribe("pump_3/homing");
      //client.subscribe("pump_master/series_length");
...
void pump_service(){
  ...
  if(tn>=next_step_1){
    if(absolute_pos_1 == requested_pos_1){ 
      ...
      client.publish("pump_3/position_now",cstr);
      //if (series_length >0){
      //  series_length--;
